...
if(isset($_POST['add_book'])){
	...
	if ($uploadOk == 0) {
	  echo "
				...
				<span class='text-sm'>Problem updating book cover, File type not valid!</span>
				</div>
			";
	// if everything is ok, try to upload file
	} else {
	  if (move_uploaded_file($_FILES["book_cover"]["tmp_name"], $target_file)) {
		$query = "INSERT INTO books (book_title, isbn_no, writer_id, group_id, book_cover, type, created_date) VALUES ('$book_title', '$isbn_no', '$writer_id', '$group_id', '$target_file', 'book', '$date')";
		if (mysqli_query($conn, $query)) {
			sleep(5);
			echo "
				<div class='alert alert-success alert-dismissible text-white' role='alert' id='success_element'>
				<span class='text-sm'>Book added succesfully!!</span>
			...
			echo "
				<div class='alert alert-danger alert-dismissible text-white' role='alert'>
				<span class='text-sm'>Book creation unsuccessful, Try Again!</span>
				</div>
			";
		}
	  } else {
		echo "
				<div class='alert alert-danger alert-dismissible text-white' role='alert'>
				<span class='text-sm'>Problem uploading book cover, Try Again!</span>
				</div>
			";
	  }
	}
  ...
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3   bg-gradient-dark" id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto  max-height-vh-100" id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link text-white active bg-gradient-primary" href="books">
            <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
        ...
        <li class="nav-item">
          <a class="nav-link text-white " href="articles_publications">
            <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
					...
					<div class="card-header pb-0 p-3">
					  <div class="row">
						<div class="col-6 d-flex align-items-center">
						  <h6 class="mb-0">Add / Manage Physical Books</h6>
						</div>
					  </div>
					</div>
				  </div>
        ...
        <div class="col-md-7 mt-4">
          <div class="card">
            ...
            <div class="card-body pt-4 p-3">
				<div class="row">
					<?php
						$query    = "SELECT * FROM `books` WHERE type = 'book' ";
						$result = mysqli_query($conn, $query);
						if (mysqli_num_rows($result) > 0) {
							// output data of each row
							while($row = mysqli_fetch_assoc($result)) {
