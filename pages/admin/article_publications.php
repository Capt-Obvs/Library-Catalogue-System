...
if(isset($_POST['add_book'])){
	...
	&& $imageFileType != "gif" ) {
	  echo "
				...
				<span class='text-sm'>Sorry, only JPG, JPEG, PNG & GIF files are allowed!</span>
				</div>
			";
	  $uploadOk = 0;
	}
	...
	if ($uploadOk == 0) {
	  echo "
				<div class='alert alert-danger alert-dismissible text-white' role='alert'>
				<span class='text-sm'>Problem updating article cover, File type not valid!</span>
				</div>
			";
	// if everything is ok, try to upload file
	} else {
	  if (move_uploaded_file($_FILES["book_cover"]["tmp_name"], $target_file)) {
		$query = "INSERT INTO books (book_title, isbn_no, writer_id, group_id, book_cover, type, created_date) VALUES ('$book_title', '$issn_no', '$writer_id', '$group_id', '$target_file', 'article', '$date')";
		if (mysqli_query($conn, $query)) {
			sleep(5);
			echo "
				<div class='alert alert-success alert-dismissible text-white' role='alert' id='success_element'>
				<span class='text-sm'>Article added succesfully!!</span>
			...
			echo "
				<div class='alert alert-danger alert-dismissible text-white' role='alert'>
				<span class='text-sm'>Article creation unsuccessful, Try Again!</span>
				</div>
			";
		}
	  } else {
		echo "
				<div class='alert alert-danger alert-dismissible text-white' role='alert'>
				<span class='text-sm'>Problem uploading article cover, Try Again!</span>
				</div>
			";
	  }
	}
  ...
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3   bg-gradient-dark" id="sidenav-main">
    ...
    <div class="collapse navbar-collapse  w-auto  max-height-vh-100" id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link text-white active bg-gradient-primary" href="article_publications">
            <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <h6 class="font-weight-bolder mb-0">Articles & Publications</h6>
        </nav>
    ...
    <div class="container-fluid py-4">
      <div class="row">
					...
					<div class="card-header pb-0 p-3">
					  <div class="row">
						<div class="col-6 d-flex align-items-center">
						  <h6 class="mb-0">Add / Manage (Academic Materials)</h6>
						</div>
					  </div>
					</div>
				  </div>
				...
				<form enctype="multipart/form-data" class="text-start" method="POST">
						...
						<div class="col-md-6">
						 ...
						 <div class="col-md-6">
						  <div class="input-group input-group-outline my-3">
							...
							<input type="text" name="issn_no" class="form-control" required>
						  </div>
						 </div>
					</div>
					<div class="row">
					...
					<div class="row">
					  ...
					  <div class="text-center">
						<button type="submit" class="btn bg-gradient-primary w-100 my-4 mb-2" name="add_book">Add</button>
					  </div>
					</form>
            </div>
          </div>
        ...
        <div class="col-md-7 mt-4">
          <div class="card">
            <div class="card-header pb-0 px-3">
              <h6 class="mb-0">Manage Books</h6>
            </div>
            <div class="card-body pt-4 p-3">
				<div class="row">
					<?php
						$query    = "SELECT * FROM `books` WHERE type = 'article' ";
						$result = mysqli_query($conn, $query);
						if (mysqli_num_rows($result) > 0) {
							// output data of each row
							while($row = mysqli_fetch_assoc($result)) {
									...
									<div class='col-md-3 col-6'>
									  <div class='card'>
										<div class='card-header p-3 text-center'>
											<img src='".$row['book_cover']."' class='position-relative' width='101' height='100' alt='pattern-tree'>
										</div>
										<div class='card-body pt-0 p-3 text-center'>
										  <h6 class='text-center text-xs'>".$row['book_title']."</h6>
										  <span class='text-xs'>".$row['isbn_no']."</span>
										  <a class='btn btn-link text-danger text-gradient px-3 mb-0' href='../includes/function?book_id=".$row['id']."'><i class='material-icons text-sm me-2'>delete</i>Delete</a>
										</div>
									  </div>
									  <hr class='horizontal dark my-3'>
									</div>
							...
							echo "
									<div class='col-md-3 col-6'>
									  <div class='card'>
										<div class='card-header p-3 text-center'>
											<img src='N/A' class='position-relative' width='101' height='100' alt='pattern-tree'>
										</div>
										<div class='card-body pt-0 p-3 text-center'>
